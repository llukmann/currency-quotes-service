...
func (w *Worker) process(ctx context.Context, claim domain.UpdateTask) {
	...
	//
	// Either way nothing can be written with a context that is done, and if it
	// is done because the service is shutting down then the task did not fail
	// at all -- we are the ones leaving. It keeps its claim and its status, and
	// the recovery pass releases it once the threshold passes.
	if err := ctx.Err(); err != nil {
		log.Warn("task left in progress",
			slog.Any("error", err),
